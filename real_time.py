...
#                 har_user = os.environ.get("HAR_USERNAME", "unknown")
#                 save_history(raw_label, display_conf, username=har_user)
#                 last_save_time = now
...
#             har_user = os.environ.get("HAR_USERNAME", "unknown")
#             cxsave_history(raw_label, display_conf, username=har_user), cy = int(lm.x * w), int(lm.y * h)
...
"""
real_time.py — Production
Real-time HAR using webcam. Saves detections under the logged-in username.
Username is passed via HAR_USERNAME environment variable from app.py.

Run: python real_time.py
     HAR_USERNAME=mihir python real_time.py
"""
...
# ── Username from environment (set by app.py when starting camera) ──
HAR_USERNAME = os.environ.get("HAR_USERNAME", "unknown")
...
# Load model & classes
# ─────────────────────────────────────────
print(f"Loading HAR model... (user: {HAR_USERNAME})")
model   = load_har_model(MODEL_PATH)
classes = np.load("classes.npy", allow_pickle=True)
print(f"Model loaded — {len(classes)} classes: {list(classes)}")
...
print(f"Starting detection for user: {HAR_USERNAME}. Press Q to quit.")
...
while cap.isOpened():
    ...
    if len(frame_buffer) == SEQ_LEN and frame_count % STEP_SIZE == 0:
        ...
        if len(pred_buffer) >= 3:
            ...
            now = time.time()
            if display_label not in ("...", "Warming up...") and (now - last_save_time) >= SAVE_INTERVAL_SEC:
                save_history(raw_label, display_conf, username=HAR_USERNAME)
                last_save_time = now
    ...
    # Username watermark
    cv2.putText(frame, f"User: {HAR_USERNAME}",
                (w - 180, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (80, 80, 100), 1, cv2.LINE_AA)

    is_risk = any(r.lower() in display_label.lower() for r in RISK_ACTIVITIES)
    color   = (60, 80, 235) if is_risk else (0, 210, 100) if display_conf >= CONFIDENCE_THRESH else (100, 100, 110)

    cv2.putText(frame, f"Activity: {display_label}",
                (20, h-62), cv2.FONT_HERSHEY_SIMPLEX, 0.85, color, 2, cv2.LINE_AA)

    if is_risk:
        cv2.putText(frame, "RISK DETECTED",
                    (20, h-30), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (60, 80, 235), 2, cv2.LINE_AA)
    ...
    cv2.imshow(f"HAR Detection — {HAR_USERNAME}  (Q to quit)", frame)
    if cv2.waitKey(1) & 0xFF == ord("q"):
        break
